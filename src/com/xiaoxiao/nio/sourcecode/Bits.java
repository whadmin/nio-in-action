...
class Bits {                            // package-private
    ...
    static void reserveMemory(long size, int cap) {
        ...
        if (!memoryLimitSet && VM.isBooted()) {
            /** 获取能够分配的最大堆外内存 参数-XX:MaxDirectMemorySize=<size>中设置 **/
        ...
        /** 尝试清理堆外内存(最终调用的是Cleaner的clean方法，其内部使用unsafe.freeMemory释放内存) **/
        while (jlra.tryHandlePendingReference()) {
            /** 判断是否存在足够的堆外内存可以供系统分配，存在直接返回**/
            if (tryReserveMemory(size, cap)) {
                return;
            }
        }

        /** 调用System.gc()来触发一次FullGC进行回收(前提是没有加-XX:-+DisableExplicitGC参数)。 **/
        System.gc();
        ...
        try {
            ...
            while (true) {
                ...
                /** 尝试清理堆外内存(最终调用的是Cleaner的clean方法，其内部使用unsafe.freeMemory释放内存),
                 *  如果失败sleep当前线程，时间是逐步增加的，规律是1, 2, 4, 8, 16, 32, 64, 128, 256
                 * **/
        ...
        SharedSecrets.setJavaNioAccess(
            new sun.misc.JavaNioAccess() {
                ...
                public ByteBuffer newDirectByteBuffer(long addr, int cap, Object ob) {
                    return new java.nio.DirectByteBuffer(addr, cap, ob);
                }
                @Override
