    ...
    implements DirectBuffer
{
    ...
    DirectByteBuffer(int cap) {                   // package-private
        ...
        super(-1, 0, cap, cap);
        /** 判断页是否对齐（操作系统中内存被分割成一块一块的页，页是内存最小单位） **/
        boolean pa = VM.isDirectMemoryPageAligned();
        /** 获取操作系统内存中页的大小（操作系统中内存被分割成一块一块的页，页是内存最小单位） **/
        int ps = Bits.pageSize();
        /** 如果是页对齐的话，申请容量实际容器需要加上一页的大小 **/
        ...
        long base = 0;
        try {
            /** 分配内存size大小的内存，并返回在堆外内存的基地址 **/
            base = unsafe.allocateMemory(size);
        } catch (OutOfMemoryError x) {
            Bits.unreserveMemory(size, cap);
            throw x;
        }
        /** 初始化起始地址base开始size个字节内存数据，从base地址位置，写入0(byte)(占用一个字节) **/
